 ...
 * @since 1.0.0
 */
if(!class_exists('SP_RTMedia_BPP_Addon')):
/**
 * @todo Description
 * @since 1.0.0
 */
class SP_RTMedia_BPP_Addon{
	/**
	 * @var string field_name_browse
	 * @since 1.0.0
	 */
	 public $field_name_browse = '_spbpp_rtmedia_browse';
	 /**
	 * @var string field_name_browse
	 * @since 1.0.0
	 */
	 public $field_name_uploads = '_spbpp_rtmedia_uploads';
	 /**
	 * @var string user_meta
	 * @since 1.0.0
	 */
	 public $user_meta_upload = '_spbpp_rtmedia_upload_access';
	  /**
	 * @var string user_meta_browse
	 * @since 1.0.0
	 */
	 public $user_meta_browse = '_spbpp_rtmedia_browse_access';
	/**
	 * SP_RTMedia_BPP_Addon initiate
	 * @since 1.0.0
	 */
	public function __construct(){
		...
		add_action('spbpp_save_product', array($this, 'save_rtmedia_access'), 12, 2);
		add_action('spbpp_order_complete', array($this, 'add_rtmedia_access'), 10, 4);
	...
	public function plugin_load_textdomain(){
		load_plugin_textdomain( 'spbpp-rtmedia', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
	}
	/**
	...
	public function spbpp_add_option($woocommerce, $post){
		//add title
		$title = __('RTMedia Option', 'spbpp-rtmedia-for-buddypress-premium');
		sbbp_section_title($title);
		//
		woocommerce_wp_checkbox(
		array(
			'id'            => $this->field_name_uploads,
			'label'         => __('Allow uploads', 'spbpp-rtmedia' ),
			'description'   => __( 'After purchase, users will be granted access to upload media with RTMedia', 'spbpp-rtmedia' ),
			)
		);
		//
		woocommerce_wp_checkbox(
		array(
			...
			'label'         => __('Allow browsing', 'spbpp-rtmedia' ),
			'description'   => __( 'After purchase, users will be granted access to view other user\s uploads with RTMedia', 'spbpp-rtmedia' ),
			)
		);
	}
	/**
	 ...
	 * @since	1.0.0
	 */
	public function save_rtmedia_access($id, $post){
	 ...
	 * @since	1.0.0
	 */
	public function add_rtmedia_access($products=array(), $user_id=0, $order_id=0, $order=array()){
		if(!empty($products)):
			foreach($products as $product){
				...
				if(!empty($field_name_browse) && $field_name_browse == 'yes'){
					if(update_user_meta($user_id, $this->user_meta_browse, 1)){
						 ...
						 */
						do_action('spbpp_rtmedia_browse_access', $user_id);
					}
				}
				//if product has media browsing enabled, let's give user access
				if(!empty($field_name_uploads) && $field_name_uploads == 'yes'){
					if(update_user_meta($user_id, $this->user_meta_upload, 1)){
						 ...
						 *	Hook can be used for email, stats, notices etc
						 */
						do_action('spbpp_rtmedia_upload_access', $user_id);
					}
				}
			}
		endif;
		...
		if($upload_access_enabled){
			...
			if(!$has_upload_access){
				...
				if($loggedin_id == $bp->displayed_user->id){
					unset($bp->bp_nav[apply_filters('rtmedia_media_tab_slug', RTMEDIA_MEDIA_SLUG )]);
				}
			}
		}
...
function load_sp_rtmedia_addon(){
	return new SP_RTMedia_BPP_Addon();
}
add_action( 'bp_premiums_loaded', 'load_sp_rtmedia_addon');
